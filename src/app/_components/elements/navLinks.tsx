...
export default function NavLinks(props: any): React.ReactElement | undefined {
    ...
    const isMatching = (data: LinkGroup, groupName: string) => {
        ...
        const foundData = linksData.find((link: any) => link.group === data.group).links.find((link: any) => {
                        ...
                        if (!data.hasOwnProperty("group")) {
                            return (
                                <Link
                                    ...
                                    key={data.key}
                                    className={pathname === `/${data.name.toLowerCase()}` ? styles.activeLink : styles.link}
                                >
                                    {data.name}
                                </Link>
                            )
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <Menu.Target>
                                        <p className={isMatching(data, pathname.split("/")[1]) ? styles.activeLink : styles.link}>
                                            {data.group}
                                        </p>
                                    </Menu.Target>
